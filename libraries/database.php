...
//            dd($sql);
    ...
    public function fetchJoneProduct($table, $sql, $page = 0, $row, $pagi = false)
    {

        $data = [];
        if ($pagi == true) {
            $total = $this->countTableProduct($table);
            $sotrang = ceil($total / $row);
            $start = ($page - 1) * $row;
            $sql .= " LIMIT $start,$row";
            $data = ["page" => $sotrang];
    ...
    public function fetchJoneTransaction($table, $sql, $page = 0, $row, $pagi = false)
    {

        $data = [];
        if ($pagi == true) {
            $total = $this->countTableTransaction($table);
            $sotrang = ceil($total / $row);
            $start = ($page - 1) * $row;
            $sql .= " LIMIT $start,$row";
            $data = ["page" => $sotrang];
    ...
    public function countTableProduct($table)
    {
        $sql = "SELECT id FROM  {$table} where user_id = ". $_SESSION['user']['id'];
        $result = mysqli_query($this->link, $sql) or die("Lỗi Truy Vấn countTable----" . mysqli_error($this->link));
        $num = mysqli_num_rows($result);
        return $num;
    }

    public function countTableTransaction($table)
    {
        $sql = "SELECT id FROM  {$table} where admin_id = ". $_SESSION['user']['id'];
        $result = mysqli_query($this->link, $sql) or die("Lỗi Truy Vấn countTable----" . mysqli_error($this->link));
        $num = mysqli_num_rows($result);
        return $num;
    }
